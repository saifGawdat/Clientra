...
import {
  ...
  Tag, Send, Calendar, TrendingUp, MessageSquare, Activity,
} from "lucide-react"
...
const STATUS_CONFIG: Record<string, { variant: any; dot: string; label: string }> = {
  ...
  return (
    <div className="p-8 space-y-6 bg-[#09090b] min-h-full">
        ...
        <div className="space-y-4">
          ...
          <div className="oled-card space-y-4">
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0">
                ...
                {contact.title && (
                  <p className="text-xs text-[#52525b] mt-0.5">{contact.title}</p>
                )}
              </div>
            </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              { icon: Activity, label: "Activities", value: contact.activities.length },
              { icon: MessageSquare, label: "Notes", value: contact.notes.length },
          ...
          {contact.tags.length > 0 && (
            <div className="oled-card space-y-2">
              <p className="text-xs font-bold uppercase tracking-widest text-[#52525b]">Tags</p>
          ...
          <Tabs defaultValue="deals">
            ...
            <TabsContent value="deals" className="space-y-2">
              {contact.deals.length === 0 ? (
                <div className="py-12 text-center text-[#52525b] text-sm">No deals linked to this contact.</div>
              ) : (
                    ...
                    <div className="oled-card flex items-center justify-between hover:border-[#7c3aed]/30 transition-colors cursor-pointer">
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-xs text-[#52525b]">{formatDate(deal.createdAt)}</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 shrink-0 ml-4">
                        ...
                        <span className={cn(
                          ...
                          DEAL_STAGE_COLORS[deal.stage] ?? DEAL_STAGE_COLORS.LEAD
                        )}>
                          {deal.stage}
                        </span>
                      </div>
            ...
            <TabsContent value="activities" className="space-y-2">
              {contact.activities.length === 0 ? (
                <div className="py-12 text-center text-[#52525b] text-sm">No activities logged for this contact.</div>
              ) : (
      ...
      <ContactFormDialog
        ...
        onSave={(updated) => { setContact((prev: any) => ({ ...prev, ...updated })); setShowEdit(false) }}
        contact={contact}
        companies={companies}
      />
    </div>
