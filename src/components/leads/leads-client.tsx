      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={stat.label} className="oled-card">
            <p className="text-[#52525b] text-xs uppercase tracking-widest mb-2">{stat.label}</p>
      ...
      <div className="border border-[#1e1e24] rounded-lg overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {filtered.map((lead) => (
              <tr key={lead.id} className="border-b border-[#1e1e24] hover:bg-[#18181b] transition-colors">
                <td className="px-4 py-3">
                  <Link href={`/contacts/${lead.id}`} className="flex items-center gap-3 group">
                    ...
                    <div>
                      ...
                      {lead.email && <p className="text-xs text-[#52525b]">{lead.email}</p>}
                    </div>
                  </Link>
                </td>
                <td className="px-4 py-3 text-[#a1a1aa]">
                  {lead.company ? (
                    <Link href={`/companies/${lead.company.id}`} className="hover:text-white transition-colors">
                      {lead.company.name}
                    </Link>
                  ) : "—"}
                </td>
                <td className="px-4 py-3 text-[#a1a1aa]">{sourceLabels[lead.source] ?? lead.source}</td>
                <td className="px-4 py-3">
                  <Badge
                ...
                <td className="px-4 py-3 text-[#52525b]">{formatDate(lead.createdAt)}</td>
                <td className="px-4 py-3">
